...
sigs <- read.table(file = "simulation_data/Genotypic.information.for.4.Additive.QTN.txt",stringsAsFactors=F,header=T, sep = "\t")[,1]
twosigs <- read.table(file = "simulation_data/Genotypic.information.for.4.Epistatic.QTN.txt",stringsAsFactors=F,header=T, sep = "\t")[,1]
...
for(i in 2:ncol(allSims)){
  ...
  sim <- read.cross(format = "csv", ".", 
                    file=paste(i-1,".genos.tmp.csv"), na.strings=c("-","NA"), genotypes = c("0","0.5","0.75","0.25","1"))
