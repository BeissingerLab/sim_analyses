...
setwd("../EMP_4_Add_QTN_4_Epi_QTN_h.2_0.6_add.eff_0_epis.eff_1_reps_100/")
##Read in and create a matrix of the simulated phenotypic data 
epistatic <- read.table("Simulated.Data.100.Reps.Herit.0.6.txt")
...
QTN2 <- QTN[,7:ncol(QTN)]
##Create matrix that identifies genotypes which carry first interacting QTN - Color
col <- matrix("black",nrow = ncol(QTN2), ncol = 1)
col[which((QTN2)[1,1:ncol(QTN2)] > 0 & (QTN2)[2,1:ncol(QTN2)])] <- "purple"
##Create matrix that identifies genotypes which carry second interacting QTN - Shape
pch <- matrix(1,nrow = ncol(QTN2), ncol = 1)
pch[which((QTN2)[1,1:ncol(QTN2)] > 0 & (QTN2)[2,1:ncol(QTN2)])] <- 0
##Create matrix that identifies genotypes which carry third interacting QTN - Size
cex <- matrix(1,nrow = ncol(QTN2), ncol = 1)
cex[which((QTN2)[5,1:ncol(QTN2)] > 0 & (QTN2)[6,1:ncol(QTN2)])] <- 2
##Create matrix that identifies genotypes which carry fourth interacting QTN - Adds X over point
...
plot(epistatic2[1:nrow(epistatic2),1], ylab = "Phenotype", xlab = "Genotype", 
     main = "Simulated Phenotypes - 0.6(5) Herit", col = col, pch = pch, cex = cex, ylim = c(0,.4))
